...
class Orderinviter extends Model {
    ...
    public function giveMoney($order_id) {
        $orderinviter_list = db('orderinviter')->where(array('orderinviter_order_id' => $order_id, 'orderinviter_valid' => 0))->lock(true)->select();
            ...
            foreach ($orderinviter_list as $val) {
                ...
                if ($goodscommon) {
                    ...
                    if (!empty($goodscommon_data)) {
                        ...
                        if (!$mysql_flag) {
                            exception('商品分销信息更新失败');
                        }
                    }
                }
                $inviter = db('inviter')->where(array('inviter_id' => $val['orderinviter_member_id']))->lock(true)->find();
				if($inviter){
					$inviter_data = array(
