    ...
    public function delay_receive() {
        ...
        if (request()->isPost()) {
            ...
            if ($update) {
                ...
                ds_show_dialog(lang('successful_delivery_deadline') . $dalay_date . '&emsp;', '', 'succ', empty(input('get.inajax')) ? '' : 'CUR_DIALOG.close();', 4);
            } else {
                ds_show_dialog(lang('delayed_failure'), '', 'succ', empty(input('get.inajax')) ? '' : 'CUR_DIALOG.close();');
            }
        } else {
