...
class Membersecurity extends BaseMember {
    ...
    public function send_bind_email() {
        ...
        if ($member_info) {
            ds_show_dialog(lang('mailbox_has_been_used'));
        }
        $seed = random(6);
        $data = array();
        $data['auth_code'] = $seed;
        ...
        if (!$update) {
            ds_show_dialog(lang('system_error'));
        }
        ...
        if ($result) {
            ...
            ds_show_dialog(lang('verify_mail_been_sent_mailbox'), url('Membersecurity/index'), 'succ', '', 5);
        } else {
            ds_show_dialog(lang('system_error'));
        }
    }
    ...
    public function modify_pwd() {
        ...
        if (TIMESTAMP - session('auth_modify_pwd') > 1800) {
            ds_show_dialog(lang('operation_timed_out'), url('Membersecurity/auth', ['type' => 'modify_pwd']), 'html', 'error');
        }
        ...
        if ($data['password'] != $data['confirm_password']) {
            ds_show_dialog(lang('two_password_inconsistencies'));
        }
    ...
    public function modify_mobile() {
        ...
        if (request()->isPost()) {
            ...
            if (!$member_common_info) {
                ds_show_dialog(lang('mobile_verification_code_error'));
            }
            if (TIMESTAMP - $member_common_info['send_acode_time'] > 1800) {
                ds_show_dialog(lang('phone_verification_code_expired'));
            }
            $data = array();
            $data['auth_code'] = '';
            $data['send_acode_time'] = 0;
            ...
            ds_show_dialog(lang('phone_number_bound_successfully'), url('Membersecurity/index'), 'succ');
        }
    }
