...
class Membervrorder extends BaseMember {
    ...
    public function change_state() {
        ...
        if (!isset($result['code'])) {
            ds_show_dialog(lang('no_permission_operation'), '', 'error');
        } else {
    ...
    public function resend() {
        ...
        if (!preg_match('/^[\d]{11}$/', input('post.buyer_phone'))) {
            ds_show_dialog(lang('please_fill_phone_number_correctly'));
        }
        $order_id = intval(input('post.order_id'));
        if ($order_id <= 0) {
            ds_show_dialog(lang('member_change_parameter_error'));
        }
        ...
        if (empty($order_info) && $order_info['order_state'] != ORDER_STATE_PAY) {
            ds_show_dialog(lang('error_order_information'));
        }
        if ($order_info['vr_send_times'] >= 5) {
            ds_show_dialog(lang('you_send_too_many_times'));
        }
        ...
        ds_show_dialog(lang('send_success'), '', 'succ', "DialogManager.close('vr_code_resend');");
    }
