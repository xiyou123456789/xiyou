...
class Memberaddress extends BaseMember {
    ...
    public function delivery_add() {
        if (request()->isPost()) {
            ...
            if (empty($info)) {
                ds_show_dialog(lang('pick_up_point_exist'), '', 'error');
            }
            $data = array();
            ...
            if (intval(input('param.address_id'))) {
                ...
                if ($count >= 20) {
                    ds_show_dialog(lang('valid_addresses_allowed'), '', 'error');
                }
            ...
            ds_show_dialog(lang('ds_common_save_succ'), url('Memberaddress/index'), 'js');
        } else {
